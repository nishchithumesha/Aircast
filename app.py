...
def value_to_colormap_color(
    ...
    """
    Map a numeric value into a matplotlib colormap (continuous heatmap).
    By default uses 'turbo'.
    """
    if vmax <= vmin:
        vmax = vmin + 1.0
...
def overlay_color_on_image(
    base_img: Image.Image,
    color_rgb: Tuple[int, int, int],
    alpha: float = 0.4
) -> Optional[Image.Image]:
    """
    Overlay a single heatmap color (RGB) with transparency 'alpha'
...
def create_colormap_legend(
    ...
    """
    Create a small horizontal legend image for the selected colormap.
    """
...
if df is not None and not df.empty:
    ...
    # Satellite heatmap overlays for each selected variable (using continuous colormap)
    if sat_img is not None and ok:
        st.subheader("Satellite heatmap overlays (per selected variable)")

        # Heatmap range and colormap settings
        HEAT_VMIN = 0.0
        HEAT_VMAX = 200.0
        HEATMAP_CMAP = "turbo"  # try 'plasma', 'inferno', 'viridis', etc.

        # Show the legend for the chosen colormap
        legend_img = create_colormap_legend(
            vmin=HEAT_VMIN,
            vmax=HEAT_VMAX,
        ...
        st.image(
            legend_img,
            caption=f"Heatmap scale ({HEAT_VMIN} → {HEAT_VMAX}) using '{HEATMAP_CMAP}' colormap",
            use_container_width=False
        )

        # Variables to visualize
        vars_to_show = [v for v in vars_sel if v in df["variable"].unique()]
        if not vars_to_show:
            vars_to_show = list(df["variable"].unique())

        n_cols = min(3, max(1, len(vars_to_show)))
        ...
        for var in vars_to_show:
            ...
            latest_val = float(d_var["value"].iloc[-1])
            unitv = d_var["unit"].mode().iat[0] if not d_var["unit"].empty else ""

            color = value_to_colormap_color(
                latest_val,
                vmin=HEAT_VMIN,
                vmax=HEAT_VMAX,
                cmap_name=HEATMAP_CMAP
            )
            overlay_img = overlay_color_on_image(
                sat_img,
                color_rgb=color,
                alpha=0.45  # transparency of heatmap
            )
            ...
            c.image(
                overlay_img,
                caption=(
                    f"{var} heatmap overlay — latest = "
                    f"{latest_val:.1f} {unitv} (mapped on {HEAT_VMIN}–{HEAT_VMAX})"
                ),
                use_container_width=True
            )
            idx += 1
